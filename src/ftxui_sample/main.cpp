...
// configuration step. It creates a namespace called `TemplateTest`. You can modify
...
struct GameBoard {
    ...
    std::string &get_string(std::size_t cur_x, std::size_t cur_y) {
        ...
        for (std::size_t cur_x = 0; cur_x < game_board.width; ++cur_x) {
            for (std::size_t cur_y = 0; cur_y < game_board.height; ++cur_y) {
                buttons.push_back(ftxui::Button(
                    &game_board.get_string(cur_x, cur_y), [=, &game_board] {
    ...
    void Render(ftxui::Screen &screen) override {
        for (std::size_t cur_x = 0; cur_x < width_; ++cur_x) {
            for (std::size_t cur_y = 0; cur_y < height_ / 2; ++cur_y) {
                auto &pixel =
                    screen.PixelAt(box_.x_min + static_cast<int>(cur_x),
                ...
                pixel.foreground_color =
                    ftxui::Color{bottom_color.R.get(), bottom_color.G.get(),
                                 bottom_color.B.get()};
            }
        }
    }
...
void game_iteration_canvas() {
    // this should probably have a `bitmap` helper function that does what
    // cur_you expect similar to the other parts of FTXUI
